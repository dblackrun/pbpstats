...
class StatsNbaEnhancedPbpLoader(StatsNbaPbpLoader, NbaEnhancedPbpLoader):
    ...
    def _make_pbp_items(self):
        ...
        self.items = [self.factory.get_event_class(item['EVENTMSGTYPE'])(item, i) for i, item in enumerate(self.data)]
    ...
    def _fix_common_event_order_error(self, exception):
        ...
        if rows[issue_event_index][event_type_index] in [8, 9]:
        ...
        elif rows[issue_event_index][event_type_index] == 18 and rows[issue_event_index + 1][event_type_index] == 4:
            ...
            self.source_data['resultSets'][0]['rowSet'][issue_event_index + 1] = replay_event
            self.source_data['resultSets'][0]['rowSet'][issue_event_index] = rebound_event

        elif rows[issue_event_index + 1][event_type_index] == 4 and rows[issue_event_index + 1][event_num_index] == int(event_num) - 1:
            # rebound and shot need to be flipped
            rebound_event = rows[issue_event_index + 1]
            shot_event = rows[issue_event_index]
            self.source_data['resultSets'][0]['rowSet'][issue_event_index] = rebound_event
            self.source_data['resultSets'][0]['rowSet'][issue_event_index + 1] = shot_event
        elif (
                rows[issue_event_index + 1][event_type_index] == 4 and rows[issue_event_index + 1][event_num_index] == int(event_num) + 2 and
                rows[issue_event_index - 1][event_type_index] == 2 and rows[issue_event_index - 1][event_num_index] == int(event_num) + 1
        ):
            ...
            self.source_data['resultSets'][0]['rowSet'][issue_event_index - 1] = rebound_event
        elif (
                rows[issue_event_index + 1][event_type_index] == 4 and rows[issue_event_index + 1][event_num_index] == int(event_num) - 2 and
                rows[issue_event_index - 1][event_type_index] == 2 and rows[issue_event_index - 1][event_num_index] == int(event_num) - 1
        ):
            ...
            shot_event = rows[issue_event_index - 1]
            self.source_data['resultSets'][0]['rowSet'][issue_event_index + 1] = second_rebound
            self.source_data['resultSets'][0]['rowSet'][issue_event_index] = shot_event
            self.source_data['resultSets'][0]['rowSet'][issue_event_index - 1] = first_rebound
