...
class TestStatsEnhancedPbpLoader:
    ...
    def test_file_loader_loads_data(self):
        pbp_loader = StatsNbaEnhancedPbpLoader(self.game_id, 'file', self.data_directory)
        ...
        assert pbp_loader.items[0].data['video_available'] == self.expected_first_item_data['video_available']
        ...
        assert pbp_loader.items[0].data['order'] == self.expected_first_item_data['order']
    ...
    def test_web_loader_loads_data(self):
        ...
        file_directory = None
        pbp_loader = StatsNbaEnhancedPbpLoader(self.game_id, 'web', file_directory)
        assert len(pbp_loader.items) == 540
        assert isinstance(pbp_loader.items[0], StatsEnhancedPbpItem)
        assert pbp_loader.items[0].data['game_id'] == self.expected_first_item_data['game_id']
        assert pbp_loader.items[0].data['event_num'] == self.expected_first_item_data['event_num']
    ...
    def test_web_loader_with_events_out_of_order_loads_data(self):
        ...
        with open(f'{self.data_directory}/game_details/stats_home_shots_{game_id}.json') as f:
            home_response = json.loads(f.read())
        with open(f'{self.data_directory}/game_details/stats_away_shots_{game_id}.json') as f:
            away_response = json.loads(f.read())
        base_url = 'https://stats.nba.com/stats/shotchartdetail'
        home_query_params = {
            'GameID': game_id,
        ...
        with open(f'{self.data_directory}/game_details/stats_summary_{game_id}.json') as f:
            summary_response = json.loads(f.read())
        summary_base_url = 'https://stats.nba.com/stats/boxscoresummaryv2'
        query_params = {'GameId': self.game_id}
        summary_url = furl(summary_base_url).add(query_params).url
        ...
        file_directory = None
        pbp_loader = StatsNbaEnhancedPbpLoader(game_id, 'web', file_directory)
