...
class TestStatsScoreboardLoader:
    ...
    def test_file_loader_loads_data(self):
        scoreboard_loader = StatsNbaScoreboardLoader(self.date, self.league, self.data_directory, 'file')
    ...
    def test_web_loader_loads_data(self):
        with open(f'{self.data_directory}/schedule/stats_{self.league}_{self.date.replace("/", "_")}.json') as f:
        ...
        scoreboard_loader = StatsNbaScoreboardLoader(self.date, self.league, self.data_directory, 'web')
        assert len(scoreboard_loader.items) == 7
        assert isinstance(scoreboard_loader.items[0], StatsNbaGameItem)
        assert scoreboard_loader.items[0].data == self.expected_first_item_data
