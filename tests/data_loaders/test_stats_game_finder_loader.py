...
class TestStatsGameFinderLoader:
    ...
    def test_file_loader_loads_data(self):
        scoreboard_loader = StatsNbaGameFinderLoader(self.league, self.season, self.season_type, 'file', self.data_directory)
    ...
    def test_web_loader_loads_data(self):
        with open(f'{self.data_directory}/schedule/stats_{self.league}_{self.season.replace("-", "_")}_{self.season_type.replace(" ", "_")}.json') as f:
        ...
        responses.add(responses.GET, scoreboard_url, json=scoreboard_response, status=200)

        scoreboard_loader = StatsNbaGameFinderLoader(self.league, self.season, self.season_type, 'web', self.data_directory)
        assert len(scoreboard_loader.items) == 1230
        assert isinstance(scoreboard_loader.items[0], StatsNbaGameItem)
        assert scoreboard_loader.items[0].data == self.expected_first_item_data
